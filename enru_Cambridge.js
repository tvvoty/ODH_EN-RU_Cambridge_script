...
class encn_Cambridge {
    ...
    async displayName() {
        ...
        return 'Cambridge EN->RU Dictionary (SC)';
    }
            ...
            for (const sensbody of sensbodys) {
                ...
                for (const sensblock of sensblocks) {
                    ...
                    for (const defblock of defblocks) {
                        ...
                        let rus_tran = T(defblock.querySelector('.def-body .trans'));
                        if (!eng_tran) continue;
                        let definition = '';
                        eng_tran = `<span class='eng_tran'>${eng_tran.replace(RegExp(expression, 'gi'),`<b>${expression}</b>`)}</span>`;
                        rus_tran = `<span class='rus_tran'>${rus_tran}</span>`;
                        let tran = `<span class='tran'>${eng_tran}${rus_tran}</span>`;
                        ...
                        if (examps.length > 0 && this.maxexample > 0) {
                            ...
                            for (const [index, examp] of examps.entries()) {
                                ...
                                let rus_examp = T(examp.querySelector('.trans'));
                                definition += `<li class='sent'><span class='eng_sent'>${eng_examp.replace(RegExp(expression, 'gi'),`<b>${expression}</b>`)}</span><span class='rus_sent'>${rus_examp}</span></li>`;
                            }
                            definition += '</ul>';
                        }
    ...
    async findYoudao(word) {
        ...
        function getYoudao(doc) {
            ...
            for (const defNode of defNodes){
                ...
                definition += `<li class="ec">${pos}<span class="ec_rus">${def}</span></li>`;
            }
            definition += '</ul>';
            let css = `
                <style>
    ...
    renderCSS() {
        let css = `
            <style>
                ...
                span.rus_tran {color:#0d47a1;}
                ul.sents {font-size:0.8em; list-style:square inside; margin:3px 0;padding:5px;background:rgba(13,71,161,0.1); border-radius:5px;}
                li.sent  {margin:0; padding:0;}
                span.eng_sent {margin-right:5px;}
                span.rus_sent {color:#0d47a1;}
            </style>`;
        return css;
    }
}
